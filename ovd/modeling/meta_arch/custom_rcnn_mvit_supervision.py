...
import numpy as np
from ..utils import draw_and_save_boxes_with_nms
...
class CustomRCNNMViT(GeneralizedRCNN):
    ...
    def inference(
        ...
        des_feats = torch.cat([i['des_feats'][1] for i in batched_inputs]).to(self.device)
        results, _ = self.roi_heads(images, (features, None), proposals, des_feats=des_feats)
        # if self.num % 40 == 0:
        #     draw_and_save_boxes_with_nms(results, batched_inputs)
        # self.num  += 1
        if do_postprocess:
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        proposals, detector_losses = self.roi_heads(
            images, (features, distill_clip_features), proposals, gt_instances, ann_type=ann_type, des_feats=des_feats)
